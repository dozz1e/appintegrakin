      ...
      <div v-if="filas.length" class="lg:hidden divide-y divide-gray-50">
        <div v-for="f in filas" :key="f.id" class="p-4">
          <div class="flex items-center justify-between gap-2">
            <NuxtLink :to="`/tickets/${f.entidad_id}`" class="font-medium text-gray-700 truncate hover:underline">
              {{ f.ticket_titulo }}
            </NuxtLink>
            <SharedBadge :label="colorTicket(f.estado_nuevo).label" :clases="colorTicket(f.estado_nuevo).clases" />
          </div>
          <p class="text-xs text-gray-500 mt-1">
            <span v-if="f.estado_anterior">Desde {{ colorTicket(f.estado_anterior).label }} · </span>{{ f.usuario_nombre ?? '—' }} · {{ formatearFechaHora(f.created_at) }}
          </p>
        </div>
      </div>
    </div>
