<!-- app/components/capacitaciones/CalendarioSemana.vue -->
...
const emit = defineEmits<{
  ...
  'editar-cita': [cita: CitaCapacitacionConNombres]
}>()
...
interface CitaUbicada {
  cita: CitaCapacitacionConNombres
  hora: number
}

const citasPorDia = computed(() => {
  const mapa = new Map<string, CitaUbicada[]>()
  ...
  for (const c of props.citas) {
    ...
    mapa.get(key)!.push({ cita: c, hora: fecha.getHours() })
  }
  return mapa
})

function citasEnCelda(diaKey: string, hora: number): CitaCapacitacionConNombres[] {
  return (citasPorDia.value.get(diaKey) ?? [])
...
function formatearHoraCita(fechaIso: string): string {
...
function onClickCita(c: CitaCapacitacionConNombres, diaKey: string) {
  emit('update:fechaSeleccionada', diaKey)
  emit('editar-cita', c)
}

const claseEstado: Record<string, string> = {
  pendiente: 'bg-primary-subtle text-primary-ink hover:bg-primary/20',
  completada: 'bg-success-bg text-success-text hover:bg-success-bg/70',
  cancelada: 'bg-danger-bg text-danger-text hover:bg-danger-bg/70 line-through',
}
</script>
      ...
      <div class="min-w-[560px]">
        ...
        <div class="max-h-[520px] overflow-y-auto">
          ...
          >
            ...
            >
              <button
                v-for="c in citasEnCelda(formatearKey(dia), hora)"
                :key="c.id"
                type="button"
                class="w-full text-left rounded-md px-2 py-1 mb-1 text-xs transition-colors"
                :class="claseEstado[c.estado]"
                @click.stop="onClickCita(c, formatearKey(dia))"
              >
                <p class="font-medium truncate">{{ c.cliente_nombre }}</p>
                <p class="text-[10px] opacity-80 truncate">{{ formatearHoraCita(c.fecha_hora) }} · {{ c.producto_nombre }}</p>
              </button>
            </div>
          </div>
        </div>
