...
<template>
  <div ref="contenedor" class="relative w-full">
    ...
    <div
      v-if="abierto"
      class="absolute mt-2 w-full max-w-md bg-white border border-gray-100 rounded-xl shadow-lg z-50 max-h-96 overflow-y-auto"
    >
      <p v-if="termino.trim().length < 2" class="text-xs text-gray-400 px-4 py-3">
        Escribe al menos 2 caracteres...
      </p>

      <p v-else-if="cargando" class="text-xs text-gray-400 px-4 py-3">Buscando...</p>

      <p v-else-if="!hayResultados" class="text-xs text-gray-400 px-4 py-3">
        Sin resultados para "{{ termino }}"
      </p>

      <template v-else>
        <div v-if="resultados.clientes.length" class="py-2">
          <p class="px-4 text-[11px] font-semibold text-gray-400 uppercase tracking-wide mb-1">Clientes</p>
          <button
            v-for="c in resultados.clientes"
            :key="c.id"
            class="w-full text-left px-4 py-2 hover:bg-gray-50 flex flex-col"
            @click="irA(`/clientes/${c.id}`)"
          >
            <span class="text-sm font-medium text-gray-800">{{ c.razon_social }}</span>
            <span class="text-xs text-gray-400">{{ c.nombre_contacto || c.email || c.telefono || '' }}</span>
          </button>
        </div>

        <div v-if="resultados.leads.length" class="py-2 border-t border-gray-100">
          <p class="px-4 text-[11px] font-semibold text-gray-400 uppercase tracking-wide mb-1">Leads</p>
          <button
            v-for="l in resultados.leads"
            :key="l.id"
            class="w-full text-left px-4 py-2 hover:bg-gray-50 flex items-center justify-between gap-2"
            @click="irA(`/leads/${l.id}`)"
          >
            <div class="flex flex-col">
              <span class="text-sm font-medium text-gray-800">{{ l.nombre }}</span>
              <span class="text-xs text-gray-400">{{ l.telefono || l.email || '' }}</span>
            </div>
            <SharedBadge :label="colorLead(l.estado).label" :clases="colorLead(l.estado).clases" />
          </button>
        </div>

        <div v-if="resultados.tickets.length" class="py-2 border-t border-gray-100">
          <p class="px-4 text-[11px] font-semibold text-gray-400 uppercase tracking-wide mb-1">Tickets</p>
          <button
          ...
          >
            <span class="text-sm font-medium text-gray-800 truncate">{{ t.titulo }}</span>
